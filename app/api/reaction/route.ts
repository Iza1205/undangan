...
// GET /api/reaction — ambil semua reactions (bisa filter by doaId)
...
// POST /api/reaction — toggle reaction (tambah atau hapus)
export async function POST(request: Request) {
  const body = await request.json()

  if (!body.doaId?.trim()) {
    return NextResponse.json({ error: 'doaId required' }, { status: 400 })
  }
  if (!body.name?.trim()) {
    return NextResponse.json({ error: 'name required' }, { status: 400 })
  }
  if (!VALID_TYPES.includes(body.type)) {
    return NextResponse.json({ error: 'type must be love | suka | senyum' }, { status: 400 })
  }

  const result = toggleReaction({
    doaId: body.doaId.trim(),
    name: body.name.trim(),
