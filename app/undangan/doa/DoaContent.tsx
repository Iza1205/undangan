  ...
  const handleRsvpSubmit = async () => {
    ...
    try {
      const r = await fetch('/api/rsvp', {
        ...
        body: JSON.stringify({ name: rsvpName.trim() || guest, attendance }),
      })
      if (r.ok) {
        setRsvpSub(true)
  ...
  return (
    <div className="app-shell">
      <style suppressHydrationWarning>{`
        ...
        .att-btn:hover:not([data-selected="true"]) {
          border-color: var(--accent) !important;
      ...
      <div className="page-content" style={{ padding: '0 20px 100px' }}>
        ...
        <div className="fade-up" style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 52, paddingBottom: 24 }}>
          <button
            onClick={() => router.back()}
            style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
          >
            <ArrowLeft size={15} color="var(--ink-2)" />
          </button>
          <div>
            <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink-1)', letterSpacing: '-0.02em' }}>
              Ucapan &amp; Konfirmasi
            </p>
            <p style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 1 }}>
              untuk {groom.name} &amp; {bride.name}
            </p>
          </div>
        </div>
        ...
        {!rsvpSubmitted ? (
          <div className="card fade-up-delay-1" style={{ padding: '18px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
              ...
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-1)', letterSpacing: '-0.02em' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
              {[{ v: 'hadir', label: '✅ Hadir' }, { v: 'tidak', label: '❌ Tidak Hadir' }].map(({ v, label }) => (
                <button key={v} onClick={() => setAtt(v as 'hadir' | 'tidak')}
                  className="att-btn"
                  data-selected={attendance === v ? 'true' : 'false'}
                  style={{
                    padding: '10px', borderRadius: 8, border: '1px solid', fontSize: 12, fontWeight: 600,
                    cursor: 'pointer', fontFamily: 'var(--font)', transition: 'all 0.15s',
                    borderColor: attendance === v ? 'var(--accent)' : 'var(--border)',
                    background:  attendance === v ? 'var(--accent-bg)' : 'var(--bg)',
                    color:       attendance === v ? 'var(--accent)' : 'var(--ink-2)',
                  }}
                >{label}</button>
              ))}
            </div>
            ...
            <button onClick={handleRsvpSubmit} disabled={!attendance || rsvpSubmitting}
              style={{
                ...
                background: attendance ? 'var(--accent)' : 'var(--border)',
                color: attendance ? '#fff' : 'var(--ink-3)',
                fontSize: 13, fontWeight: 600, cursor: attendance && !rsvpSubmitting ? 'pointer' : 'not-allowed',
                fontFamily: 'var(--font)', transition: 'all 0.15s', opacity: rsvpSubmitting ? 0.7 : 1,
              }}
            >
              <Send size={13} />
        ...
        {!doaSubmitted ? (
          <div className="card fade-up-delay-2" style={{ padding: '18px', marginBottom: 16 }}>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Semoga selalu bahagia dan diberkahi... 🙏"
              value={message}
            ...
            <button onClick={handleDoaSubmit} disabled={!message.trim() || doaSubmitting}
              style={{
                ...
                color: message.trim() ? '#fff' : 'var(--ink-3)',
                fontSize: 13, fontWeight: 600,
                cursor: message.trim() && !doaSubmitting ? 'pointer' : 'not-allowed',
                fontFamily: 'var(--font)', transition: 'all 0.15s', opacity: doaSubmitting ? 0.7 : 1,
              }}
            >
              <Heart size={13} />
        ...
        <div className="fade-up-delay-3" style={{ marginTop: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            {!feedLoading && doas.length > 0 && (
              <button
                onClick={() => feedBottomRef.current?.scrollIntoView({ behavior: 'smooth' })}
                style={{
                  flexShrink: 0, width: 28, height: 28, borderRadius: 8,
                  border: '1px solid var(--border)', background: 'var(--surface)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  cursor: 'pointer', animation: 'bounceY 1.6s ease-in-out infinite',
                }}
              >
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderColor: isNew ? 'var(--accent)' : undefined,
                      opacity: 0,
                      animation: isNew
                        ? 'slideDown 0.35s ease forwards'
                        : `fadeUp 0.35s ease ${idx * 50}ms forwards`,
                    }}
                  >
                    <div style={{
