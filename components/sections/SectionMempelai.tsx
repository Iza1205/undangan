...
import MempelaiSheet from './MempelaiSheet'
...
function PersonCard({ photo, name, father, mother, instagram, animationDelay = '0s', onClick }: PersonCardProps) {
  return (
    <div
    ...
    >
      <div style={{ width: 72, height: 72, borderRadius: 16, overflow: 'hidden', marginBottom: 10, position: 'relative', flexShrink: 0 }}>
        <Image src={photo} alt={name} fill sizes="72px" style={{ objectFit: 'cover', objectPosition: 'center top' }} />
      </div>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10, fontWeight: 700, color: 'var(--accent)', background: 'var(--accent-bg)', padding: '4px 10px', borderRadius: 20, marginBottom: 12 }}>
      ...
      <a
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 10, fontWeight: 700, color: '#dc2743', background: '#fff0f3', padding: '5px 10px', borderRadius: 20, textDecoration: 'none', letterSpacing: '0.01em', border: '1px solid #fbc8d4' }}
      >
        <IgIcon size={13} />
        @{instagram}
      </a>
...
export default function SectionMempelai({ q }: { q: string }) {
  ...
  const groomData = { ...groom, instagram: 'izamhn',   photo: '/mempelai/1.png' }
  const brideData = { ...bride, instagram: 'fkrnurull', photo: '/mempelai/2.png' }

  return (
    <div style={{ padding: '0 20px', marginTop: 16 }} className="fade-up fade-up-delay-2">
      <style>{`
        .mempelai-card { animation: mempelai-breathe 3s ease-in-out infinite; }
      ...
      {selected === 'groom' && (
        <MempelaiSheet person={groomData as any} isGroom={true} onClose={() => setSelected(null)} />
      )}
      {selected === 'bride' && (
        <MempelaiSheet person={brideData as any} isGroom={false} onClose={() => setSelected(null)} />
      )}
    </div>
  )
}